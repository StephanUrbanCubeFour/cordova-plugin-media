...
{
    NSLog(@"urlForRecording - %@",resourcePath);
...
{
    NSLog(@"urlForPlaying - %@",resourcePath);
    NSURL* resourceURL = nil;
    NSString* filePath = nil;
...
{
    NSLog(@"audioFileForResource - %@ - %@",resourcePath,mediaId);
    BOOL bError = NO;
...
#pragma unused(callbackId)
    ...
    if ((audioFile != nil) && (audioFile.resourceURL != nil)) {
        void (^startRecording)(void) = ^{
			...
			/**********************************************************************************/
            //Replaced to record MP4 instead of WAV see:
...
{
    NSLog(@"audioRecorderDidFinishRecording - %i",flag);
